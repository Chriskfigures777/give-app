  ...
  return (
    <div className="space-y-6 p-2 sm:p-4">
      <div className="dashboard-fade-in">
        <h1 className="text-2xl font-bold tracking-tight text-dashboard-text">Donations</h1>
        <p className="mt-1 text-dashboard-text-muted">All donations received by your organization.</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="dashboard-fade-in dashboard-fade-in-delay-1 rounded-2xl border border-dashboard-border bg-dashboard-card p-5 shadow-sm transition-shadow hover:shadow-md">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-2xl font-bold text-dashboard-text">${(totalCents / 100).toLocaleString()}</p>
            </div>
        ...
        <div className="dashboard-fade-in dashboard-fade-in-delay-2 rounded-2xl border border-dashboard-border bg-dashboard-card p-5 shadow-sm transition-shadow hover:shadow-md">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-2xl font-bold text-dashboard-text">{totalDonors}</p>
            </div>
        ...
        <div className="dashboard-fade-in dashboard-fade-in-delay-3 rounded-2xl border border-dashboard-border bg-dashboard-card p-5 shadow-sm transition-shadow hover:shadow-md">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-sm font-medium text-dashboard-text-muted">Successful</p>
              <p className="mt-1 text-2xl font-bold text-dashboard-text">{succeededCount}</p>
            </div>
      ...
      <div className="dashboard-fade-in dashboard-fade-in-delay-4 rounded-2xl border border-dashboard-border bg-dashboard-card overflow-hidden shadow-sm">
        <div className="border-b border-dashboard-border px-5 py-4">
          <h3 className="text-base font-bold text-dashboard-text">All Donations</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-dashboard-card-hover/50">
              <tr>
                <th className="text-left p-3 font-semibold text-dashboard-text-muted">Amount</th>
                <th className="text-left p-3 font-semibold text-dashboard-text-muted">Giver</th>
                <th className="text-left p-3 font-semibold text-dashboard-text-muted">Organization</th>
                <th className="text-left p-3 font-semibold text-dashboard-text-muted">Campaign</th>
                <th className="text-left p-3 font-semibold text-dashboard-text-muted">Status</th>
                <th className="text-left p-3 font-semibold text-dashboard-text-muted">Date</th>
              </tr>
            </thead>
            <tbody>
              {donations.map((d) => (
                <tr key={d.id} className="border-t border-dashboard-border transition-colors hover:bg-dashboard-card-hover/50">
                  <td className="p-3 text-right font-medium text-dashboard-text">
                    ${(Number(d.amount_cents) / 100).toFixed(2)} {d.currency}
                  </td>
                  <td className="p-3 text-dashboard-text">
                  ...
                  <td className="p-3">
                    <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${d.status === "succeeded" ? "bg-emerald-500/15 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400" : d.status === "pending" ? "bg-amber-500/15 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400" : "bg-red-500/15 text-red-700 dark:bg-red-500/20 dark:text-red-400"}`}>
                      {d.status}
                    </span>
                  </td>
                  <td className="p-3 text-dashboard-text-muted">
