...
export function DonationForm({
  ...
  const basePath = isEmbed ? `/give/${slug}/embed` : `/give/${slug}`;
  ...
  return (
    <div id="checkout" style={{ ...formStyle, background: formStyle ? "#fff" : undefined }}>
      <div
        className="give-form-tithely"
        style={{
          background: formBg,
          borderRadius: radius,
          padding: "24px",
      ...
      >
        ...
        {/* Prominent amount display — Tithe.ly style */}
        ...
        {/* Amount presets — segmented style + any amount */}
        <div className="mb-5">
                ...
                <div className="mt-3">
                  <div className="flex justify-between text-xs text-slate-600 mb-1">
                    ...
                    <span>
                      $
                      {(
                        (Number(selectedCampaign.current_amount_cents ?? 0) / 100).toLocaleString()
                      )}{" "}
                      / $
        ...
        <div className="mb-4 space-y-3">
          {showAnonymousOption && (
            <div className="flex items-center gap-2">
              ...
              />
              <label htmlFor="give-anonymously" className="text-sm font-medium text-slate-700 cursor-pointer">
                Give anonymously
              </label>
            </div>
          )}
          {/* Payment frequency: always show so donors can choose one-time, monthly, or yearly */}
          <div className="space-y-2">
            <span className="block text-sm font-medium text-slate-700">Payment frequency</span>
            <div className="flex flex-wrap gap-2">
              {[
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (disabled) return;
                      // Monthly/yearly require login — redirect immediately so they can sign in, then return to complete payment
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">
              Email <span className="text-slate-500 font-normal">(required for tax receipts)</span>
            </label>
            <input
              type="email"
              value={donorEmail}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">
              Name {isAnonymous ? "(hidden when anonymous)" : "(optional)"}
            </label>
            <input
              type="text"
              value={donorName}
        ...
        {isAnonymous && (
          <p className="text-sm text-slate-500 mb-4">
            Your name will not be shared with the organization. Email is required for tax receipts and donation records.
          </p>
        )}

        {/* Payment — next step hint */}
        <h3 className={sectionHeading}>Payment</h3>
        <p className="text-sm text-slate-500 mb-4">
          You&apos;ll add your payment method on the next step.
        </p>

        {/* Transaction fee — clear options */}
        <div className="mb-5">
          <h3 className={sectionHeading}>
            Transaction fee
            <span className="ml-1.5 text-slate-400 font-normal" title="Card processing (2.9% + 30¢) and platform fee (1%). You can choose who pays.">
              (i)
            </span>
          </h3>
          <p className="text-sm text-slate-600 mb-3">
            A transaction fee applies. You can cover it so the organization receives your full donation, or the organization can absorb it.
          </p>
          <div className="space-y-2">
            {(["org_pays", "donor_platform", "donor_stripe", "donor_both"] as const).map((opt) => {
              ...
              return (
                ...
                >
                  ...
                  {fee > 0 && (
                    <span style={{ opacity: selected ? 0.95 : 0.8 }}>
                      +${(fee / 100).toFixed(2)}
                    </span>
                  )}
                </button>
        ...
        {/* Checkout summary — clear total before pay */}
        <div
        ...
        >
          ...
          {feeCents > 0 && (
            <div className="flex justify-between text-sm text-slate-600 mb-1">
              <span>Transaction fee (you cover)</span>
        ...
        {/* Primary CTA — Tithe.ly style: large, dark, lock icon */}
        <button
          type="button"
          onClick={goToPayment}
