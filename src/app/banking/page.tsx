...
export default function BankingPage() {
  ...
  return (
    <>
      ...
      {/* Full-height below the sticky nav */}
        ...
        {!loading && !token && (
          <div className="flex flex-1 flex-col items-center justify-center gap-6 px-4 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Link
                href="/login"
                className="rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 px-6 py-2.5 text-sm font-extrabold text-white shadow-lg shadow-emerald-500/20 hover:opacity-90 transition-opacity"
              >
                Log in to Banking
              </Link>
              <Link
                href="/signup"
                className="rounded-xl border border-slate-200 bg-white px-6 py-2.5 text-sm font-extrabold text-slate-700 hover:bg-slate-50 transition-colors"
              >
                Create account
              </Link>
            </div>
