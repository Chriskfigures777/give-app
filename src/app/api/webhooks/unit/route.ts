...
export async function POST(req: NextRequest) {
  ...
  for (const event of events) {
    ...
    // userIds are JWT sub values: Supabase UUID or Auth0 sub (e.g. auth0|xxx)
    for (const userId of userIds) {
      const isAuth0 = userId.startsWith("auth0|");
      if (isAuth0) {
        const { data: byAuth0 } = await supabase
          .from("user_profiles")
          .select("id")
        ...
        if (byAuth0) {
          const { error } = await supabase
            ...
            .eq("id", byAuth0.id);
          if (error) console.error("[unit-webhook] Failed to update by auth0_user_id", userId, error);
        }
      } else {
        ...
        if (existing) {
          ...
          if (error) console.error("[unit-webhook] Failed to insert user_profile", userId, error);
        }
      }
    }
  }
