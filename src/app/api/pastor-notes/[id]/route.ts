...
export async function PATCH(
  ...
  try {
    ...
    const hasCoverFields = "cover_url" in body || "cover_type" in body;
    if (hasCoverFields) {
      updates.cover_url = body.cover_url ?? null;
      updates.cover_type = body.cover_type ?? null;
    }
    ...
    if (error && hasCoverFields && error.message?.includes("cover")) {
      const { cover_url: _cu, cover_type: _ct, ...safeUpdates } = updates as Record<string, unknown> & { cover_url?: unknown; cover_type?: unknown };
