-- Add missionary-related columns to user_profiles
ALTER TABLE public.user_profiles
  ADD COLUMN IF NOT EXISTS plans_to_be_missionary boolean DEFAULT false,
  ADD COLUMN IF NOT EXISTS is_missionary boolean DEFAULT false,
  ADD COLUMN IF NOT EXISTS missionary_sponsor_org_id uuid REFERENCES public.organizations(id) ON DELETE SET NULL;

-- Add missionary to allowed roles in handle_new_user
...
COMMENT ON COLUMN public.user_profiles.plans_to_be_missionary IS 'Whether the giver indicated they plan to be a missionary during signup';
COMMENT ON COLUMN public.user_profiles.is_missionary IS 'True when the user has been added as a missionary by an organization (connected via missionary_sponsor_org_id)';
COMMENT ON COLUMN public.user_profiles.missionary_sponsor_org_id IS 'Organization that sponsors this missionary. When set, user can receive donations via org embed with splits.';
